...
exports.createMessage = async (req, res) => {
  try {
    ...
    const message = toMessage(row, req.user.id);
    const conversation = await ChatConversation.findByPk(req.params.conversationId);
    if (conversation) {
      sendToUser(conversation.buyer_id, "chat:message", message);
      if (conversation.seller_id) sendToUser(conversation.seller_id, "chat:message", message);
    }
